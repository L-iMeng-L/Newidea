#!/bin/bash
# Ablation: bilinear upsample vs ConvTranspose2d
# fold1 only, cuda:2
set -euo pipefail
OUTDIR="./output/ablation_blinear"
GPU=0
VAL_FOLD=1
NAME="fold${VAL_FOLD}"

mkdir -p "$OUTDIR/${NAME}"
echo "===== Ablation: BILINEAR upsample (no ConvTranspose2d) fold1 cuda:2 ====="
python -u train.py \
    --epochs 300 --batch_size 48 --val_fold $VAL_FOLD \
    --device "cuda:${GPU}" --output_dir "$OUTDIR" --run_name "$NAME" \
    --encoder uni2-h --decoder shared_unet --full_unfreeze --upsample_mode bilinear \
