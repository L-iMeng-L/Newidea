#!/bin/bash
# run19_3fold_mala.sh — 3-fold CV: shared_unet_mala (/4+/8 only), 2:2:2, 300 epochs
# cuda:1 sequential
# ==============================================================================
set -euo pipefail
...
OUTDIR="./output/sharedunet_3fold_mala"
EPOCHS=300                # target total epochs
UNI2_WTS="/home/lwy/Newidea/pytorch_model.bin"
SEED=114514
GPU=1   # run19 on cuda:1
DECODER="shared_unet_mala"
...
run_fold() {
    ...
    echo ""
    echo "============================================================"
    echo "  [3-fold MALA]  val=fold${val_fold}  GPU=${GPU}"
...
# ================================================================
# Train + PP search per fold (sequential on cuda:1)
# ================================================================
echo "===== 3-fold CV MALA (cuda:1 sequential) ====="
for f in 2 3; do
    run_fold $f
    sleep 50
done
