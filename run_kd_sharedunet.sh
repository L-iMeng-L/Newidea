#!/bin/bash
# run_kd_run18.sh — KD: run18 teacher (shared_unet) → ConvNeXt-Tiny student, 3-fold
# cuda:2 sequential, no PP search
# ==============================================================================
set -euo pipefail
...
OUTDIR="./output/kd_sharedunet_3fold"
TEACHER_DIR="./output/sharedunet_3fold"
EPOCHS=300
SEED=114514
GPU=2
...
run_fold() {
    ...
    echo ""
    echo "============================================================"
    echo "  [KD shared_unet]  val=fold${val_fold}  GPU=${GPU}"
    ...
    python -u kd_train.py \
        --teacher_ckpt "$teacher_ckpt" --teacher_decoder shared_unet \
...
echo "===== KD (shared_unet teacher) Tiny 3-fold (cuda:2 sequential) ====="
for f in 1 2 3; do
    run_fold $f
    sleep 30
done
