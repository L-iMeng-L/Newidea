#!/bin/bash
# Ablation: unet3 (independent decoders) vs shared_unet
# fold1 only, cuda:3
set -euo pipefail
OUTDIR="./output/ablation_unet3"
GPU=1
VAL_FOLD=1
NAME="fold${VAL_FOLD}"

mkdir -p "$OUTDIR/${NAME}"
echo "===== Ablation: unet3 independent decoders (18.8M) fold1 cuda:3 ====="
python -u train.py \
    --epochs 300 --batch_size 32 --val_fold $VAL_FOLD \
    --device "cuda:${GPU}" --output_dir "$OUTDIR" --run_name "$NAME" \
    --encoder uni2-h --decoder unet3 --full_unfreeze \
    --heavy_aug --backbone_lr_mult 0.1 --enc_dropout 0.5 --dec_dropout 0.2 \
    --balance_sample --uni2_weights "/home/lwy/Newidea/pytorch_model.bin" \
